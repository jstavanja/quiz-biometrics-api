    ...
    .then(() => {
      ...
      axios.post('http://localhost:8000/student/register', formData)
        .then(() => {
          ...
          alert('something went wrong, please retry this test by refreshing the page')
        })
    })
  })
}
...
const startVideo = () => {
    if (!navigator.getMedia) {
        ...
        navigator.getMedia({video: true }, (str) => {
                track = str.getTracks()[0]
                video.src = window.URL.createObjectURL(str)
                video.play()
                video.onplay = () => {
                    showVideo()
                }
            }, () => {
                alert("Cannot access data stream for capturing the webcam")
            }
        )
    }
}
